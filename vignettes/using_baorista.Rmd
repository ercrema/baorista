...
_Baorista_ is currently not available on CRAN but can be installed via `devtools` directly from this repository with the following command:
...
```{r}
#Compute 95% HPDI with coda package
library(coda)
mcmc(exponential.fit$posterior.r) |> HPDinterval(prob=0.89)
#Plot with  bayesplot package:
library(bayesplot)
library(ggplot2)
plot_title <- ggtitle("Posterior of Growth Rate","with medians and 89% intervals")
mcmc_areas(exponential.fit$posterior.r,prob=0.89) + plot_title
```
